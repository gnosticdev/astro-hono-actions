...
describe('Integration Tests', () => {
	...
	describe('Default App', () => {
		...
		it('should have the correct routes', () => {
			expect(
				defaultApp.getPath(new Request('http://localhost/api/testAction')),
			).toBe('/api/testAction')
			expect(
				defaultApp.getPath(new Request('http://localhost/api/anotherAction')),
	...
	describe.each([
		...
		test('should generate valid router code for %s', () => {
	...
	describe('Virtual Imports Integration', () => {
		it('should use consistent virtual import IDs', () => {
			expect(VIRTUAL_MODULE_ID_CLIENT).toBe('@gnosticdev/hono-actions/client')
	...
	describe('Custom Hono instances (non-defineHonoAction)', () => {
		it('should not accept POST on a GET-only custom Hono route when mixed into honoActions', async () => {
			...
			const getOnlyRoute = getOnlyApp.get('/', (c) => c.json({ ok: true }))
			...
			type AdditionalActions = MergeActionKeyIntoPath<typeof modifiedActions>
	...
	describe('Integration Setup Behavior', () => {
		...
		beforeEach(async () => {
			...
			tempDir = await fs.mkdtemp(path.join(tmpdir(), 'hono-actions-test-'))
		...
		it('should warn and return early when no action patterns are found', async () => {
			...
			const files = await glob(
				ACTION_PATTERNS.map((p) => path.join(emptyDir, p)),
				{
					cwd: emptyDir,
			...
			expect(consoleWarnCalls[0]).toContain('No actions found. Create one of:')
		...
		it('should continue setup when action files are found', async () => {
			...
			const actionFile = path.join(projectDir, 'src', 'hono', 'actions.ts')
		...
		it('should handle custom actionsPath option correctly', async () => {
			...
			await fs.writeFile(customActionFile, '// Custom action file', 'utf-8')
		...
		it('should warn and return early when actions.ts is found in node_modules', async () => {
			// Create a project directory with node_modules
			const projectDir = path.join(tempDir, 'project-with-node-modules')
			await fs.mkdir(projectDir, { recursive: true })
			await fs.mkdir(path.join(projectDir, 'node_modules'), {
				recursive: true,
			})

			// Create an actions.ts file inside node_modules (simulating a package that might have one)
			const nodeModulesActionFile = path.join(
				projectDir,
				'node_modules',
				'some-package',
				'src',
			...
			await fs.mkdir(path.dirname(nodeModulesActionFile), {
				recursive: true,
			})
			await fs.writeFile(
				nodeModulesActionFile,
				'// Actions file in node_modules',
				'utf-8',
			)

			// Simulate finding a file in node_modules (could happen with custom actionsPath option
			// or edge cases where glob might match files in node_modules)
			// In the actual integration, this would be: const actionsPath = options.actionsPath ?? files[0]
			// For this test, we'll simulate a scenario where a file path includes 'node_modules'
			const actionsPathInNodeModules = nodeModulesActionFile

			// Simulate the check from integration.ts line 109
			if (
				!actionsPathInNodeModules ||
				actionsPathInNodeModules.includes('node_modules')
			) {
				const warningMessage = `No actions found. Create one of:\n${ACTION_PATTERNS.map((p) => ` - ${p}`).join('\n')}`
				console.warn(warningMessage)
			}
